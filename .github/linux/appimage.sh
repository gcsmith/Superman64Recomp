...
cp Superman64Recompiled AppDir/usr/bin/
cp -r assets/ AppDir/usr/bin/
cp recompcontrollerdb.txt AppDir/usr/bin/
cp icons/512.png AppDir/Superman64Recompiled.png
cp .github/linux/Superman64Recompiled.desktop AppDir/

"./linuxdeploy-$LINUX_DEPLOY_ARCH.AppImage" --appimage-extract
mv squashfs-root/ deploy
./deploy/AppRun --appdir=AppDir/ -d AppDir/Superman64Recompiled.desktop -i AppDir/Superman64Recompiled.png -e AppDir/usr/bin/Superman64Recompiled --plugin gtk
...
echo '    APP_FOLDER_PATH=$APP_FOLDER_PATH ./Superman64Recompiled' >> AppDir/AppRun
echo 'else' >> AppDir/AppRun
echo '    cd "$this_dir"/usr/bin/' >> AppDir/AppRun
echo '    ./Superman64Recompiled' >> AppDir/AppRun
