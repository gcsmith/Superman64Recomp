...
void superman64::open_quit_game_prompt() {
    recompui::open_choice_prompt(
...
struct ControlOptionsContext {
    ...
    superman64::TargetingMode targeting_mode;
    recomp::BackgroundInputMode background_input_mode;
    superman64::AutosaveMode autosave_mode;
    superman64::CameraInvertMode camera_invert_mode;
    superman64::AnalogCamMode analog_cam_mode;
    superman64::CameraInvertMode analog_camera_invert_mode;
};
...
superman64::TargetingMode superman64::get_targeting_mode() {
    return control_options_context.targeting_mode;
}

void superman64::set_targeting_mode(superman64::TargetingMode mode) {
...
superman64::AutosaveMode superman64::get_autosave_mode() {
    return control_options_context.autosave_mode;
}

void superman64::set_autosave_mode(superman64::AutosaveMode mode) {
...
void superman64::set_camera_invert_mode(superman64::CameraInvertMode mode) {
...
void superman64::set_analog_cam_mode(superman64::AnalogCamMode mode) {
...
void superman64::set_analog_camera_invert_mode(superman64::CameraInvertMode mode) {
...
void superman64::reset_sound_settings() {
...
void superman64::set_main_volume(int volume) {
...
int superman64::get_main_volume() {
    return sound_options_context.main_volume.load();
}

void superman64::set_bgm_volume(int volume) {
...
void superman64::set_low_health_beeps_enabled(bool enabled) {
...
bool superman64::get_low_health_beeps_enabled() {
...
struct DebugContext {
    ...
    DebugContext() {
        for (const auto& area : superman64::game_warps) {
    ...
    void update_warp_names() {
        scene_names.clear();
        for (const auto& scene : superman64::game_warps[area_index].scenes) {
            scene_names.emplace_back(scene.name);
        }
        
        entrance_names = superman64::game_warps[area_index].scenes[scene_index].entrances;
    }
};
...
public:
    ...
    void load_document() override {
		config_context = recompui::create_context(superman64::get_asset_path("config_menu.rml"));
    ...
    void register_events(recompui::UiEventListenerInstancer& listener) override {
        ...
        recompui::register_event(listener, "open_quit_game_prompt",
            [](const std::string& param, Rml::Event& event) {
                superman64::open_quit_game_prompt();
            });
        ...
        recompui::register_event(listener, "do_warp",
            [](const std::string& param, Rml::Event& event) {
                superman64::do_warp(debug_context.area_index, debug_context.scene_index, debug_context.entrance_index);
            });

        recompui::register_event(listener, "set_time",
            [](const std::string& param, Rml::Event& event) {
                superman64::set_time(debug_context.set_time_day, debug_context.set_time_hour, debug_context.set_time_minute);
            });
    }
    ...
    void make_controls_bindings(Rml::Context* context) {
        ...
        constructor.BindEventCallback("reset_input_bindings_to_defaults",
            [](Rml::DataModelHandle model_handle, Rml::Event& event, const Rml::VariantList& inputs) {
                if (cur_device == recomp::InputDevice::Controller) {
                    superman64::reset_cont_input_bindings();
                } else {
                    superman64::reset_kb_input_bindings();
                }
        ...
        constructor.BindEventCallback("reset_single_input_binding_to_default",
            [](Rml::DataModelHandle model_handle, Rml::Event& event, const Rml::VariantList& inputs) {
                recomp::GameInput input = static_cast<recomp::GameInput>(inputs.at(0).Get<size_t>());
                superman64::reset_single_input_binding(cur_device, input);
                model_handle.DirtyVariable("inputs");
                nav_help_model_handle.DirtyVariable("nav_help__accept");
                nav_help_model_handle.DirtyVariable("nav_help__exit");
            });
...
bool superman64::get_debug_mode_enabled() {
    return debug_context.debug_enabled;
}

void superman64::set_debug_mode_enabled(bool enabled) {
...
void recompui::update_supported_options() {
    msaa2x_supported = superman64::renderer::RT64MaxMSAA() >= RT64::UserConfiguration::Antialiasing::MSAA2X;
    msaa4x_supported = superman64::renderer::RT64MaxMSAA() >= RT64::UserConfiguration::Antialiasing::MSAA4X;
    msaa8x_supported = superman64::renderer::RT64MaxMSAA() >= RT64::UserConfiguration::Antialiasing::MSAA8X;
    sample_positions_supported = superman64::renderer::RT64SamplePositionsSupported();
