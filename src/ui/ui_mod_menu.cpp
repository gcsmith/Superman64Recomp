...
void ModMenu::open_install_dialog() {
    superman64::open_file_dialog_multiple([](bool success, const std::list<std::filesystem::path>& paths) {
        if (success) {
...
bool ModMenu::handle_special_config_options(const recomp::mods::ConfigOption& option, const recomp::mods::ConfigValueVariant& config_value) {
    if (superman64::renderer::is_texture_pack_enable_config_option(option, true)) {
...
void ModMenu::mod_hd_textures_enabled_changed(uint32_t value) {
    if (active_mod_index >= 0) {
        if (value) {
            superman64::renderer::secondary_enable_texture_pack(mod_details[active_mod_index].mod_id);
        }
        else {
            superman64::renderer::secondary_disable_texture_pack(mod_details[active_mod_index].mod_id);
        }
    }
}
...
ModMenu::ModMenu(Element *parent) : Element(parent) {
    ...
    sub_menu_context = recompui::create_context(superman64::get_asset_path("config_sub_menu.rml"));
    sub_menu_context.open();
