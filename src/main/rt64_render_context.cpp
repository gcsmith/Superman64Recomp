...
superman64::renderer::RT64Context::RT64Context(uint8_t* rdram, ultramodern::renderer::WindowHandle window_handle, bool debug) {
...
superman64::renderer::RT64Context::~RT64Context() = default;

void superman64::renderer::RT64Context::send_dl(const OSTask* task) {
...
void superman64::renderer::RT64Context::update_screen() {
    app->updateScreen();
}

void superman64::renderer::RT64Context::shutdown() {
    if (app != nullptr) {
        app->end();
    }
}

bool superman64::renderer::RT64Context::update_config(const ultramodern::renderer::GraphicsConfig& old_config, const ultramodern::renderer::GraphicsConfig& new_config) {
...
void superman64::renderer::RT64Context::enable_instant_present() {
...
uint32_t superman64::renderer::RT64Context::get_display_framerate() const {
    return app->presentQueue->ext.sharedResources->swapChainRate;
}

float superman64::renderer::RT64Context::get_resolution_scale() const {
...
void superman64::renderer::RT64Context::check_texture_pack_actions() {
...
RT64::UserConfiguration::Antialiasing superman64::renderer::RT64MaxMSAA() {
    return device_max_msaa;
}

std::unique_ptr<ultramodern::renderer::RendererContext> superman64::renderer::create_render_context(uint8_t* rdram, ultramodern::renderer::WindowHandle window_handle, bool developer_mode) {
    return std::make_unique<superman64::renderer::RT64Context>(rdram, window_handle, developer_mode);
}

bool superman64::renderer::RT64SamplePositionsSupported() {
    return sample_positions_supported;
}

bool superman64::renderer::RT64HighPrecisionFBEnabled() {
    return high_precision_fb_enabled;
}

void superman64::renderer::trigger_texture_pack_update() {
    texture_pack_action_queue.enqueue(TexturePackUpdateAction{});
}

void superman64::renderer::enable_texture_pack(const recomp::mods::ModContext& context, const recomp::mods::ModHandle& mod) {
    ...
    auto find_it = config_schema.options_by_id.find(superman64::renderer::special_option_texture_pack_enabled);
    if (find_it != config_schema.options_by_id.end()) {
        ...
        if (is_texture_pack_enable_config_option(config_option, false)) {
            ...
            if (value) {
                superman64::renderer::secondary_enable_texture_pack(mod.manifest.mod_id);
            }
            else {
                superman64::renderer::secondary_disable_texture_pack(mod.manifest.mod_id);
            }
        }
    }
}

void superman64::renderer::disable_texture_pack(const recomp::mods::ModHandle& mod) {
    texture_pack_action_queue.enqueue(TexturePackDisableAction{mod.manifest.mod_id});
}

void superman64::renderer::secondary_enable_texture_pack(const std::string& mod_id) {
    texture_pack_action_queue.enqueue(TexturePackSecondaryEnableAction{mod_id});
}

void superman64::renderer::secondary_disable_texture_pack(const std::string& mod_id) {
...
bool superman64::renderer::is_texture_pack_enable_config_option(const recomp::mods::ConfigOption& option, bool show_errors) {
    if (option.id == superman64::renderer::special_option_texture_pack_enabled) {
        if (option.type != recomp::mods::ConfigOptionType::Enum) {
            if (show_errors) {
                recompui::message_box(("Mod has the special config option id for enabling an HD texture pack (\"" + superman64::renderer::special_option_texture_pack_enabled + "\"), but the config option is not an enum.").c_str());
            }
            return false;
        }
        ...
        if (option_enum.options.size() != 2) {
            if (show_errors) {
                recompui::message_box(("Mod has the special config option id for enabling an HD texture pack (\"" + superman64::renderer::special_option_texture_pack_enabled + "\"), but the config option doesn't have exactly 2 values.").c_str());
            }
            return false;
        }
