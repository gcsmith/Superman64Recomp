...
std::filesystem::path superman64::get_app_folder_path() {
   ...
   const auto app_bundle_path = superman64::get_bundle_directory().parent_path();
   ...
   if (result == S_OK) {
       recomp_dir = std::filesystem::path{known_path} / superman64::program_id;
   }
...
#if defined(__APPLE__)
   const auto supportdir = superman64::get_application_support_directory();
   if (supportdir) {
       return *supportdir / superman64::program_id;
   }
#endif
   ...
   if (homedir != nullptr) {
       recomp_dir = std::filesystem::path{homedir} / (std::u8string{u8".config/"} + std::u8string{superman64::program_id});
   }
#endif
...
bool save_general_config(const std::filesystem::path& path) {
    ...
    superman64::to_json(config_json["targeting_mode"], superman64::get_targeting_mode());
    ...
    config_json["autosave_mode"] = superman64::get_autosave_mode();
    config_json["camera_invert_mode"] = superman64::get_camera_invert_mode();
    config_json["analog_cam_mode"] = superman64::get_analog_cam_mode();
    config_json["analog_camera_invert_mode"] = superman64::get_analog_camera_invert_mode();
    config_json["debug_mode"] = superman64::get_debug_mode_enabled();

    return save_json_with_backups(path, config_json);
}

void set_general_settings_from_json(const nlohmann::json& config_json) {
    superman64::set_targeting_mode(from_or_default(config_json, "targeting_mode", superman64::TargetingMode::Switch));
    ...
    superman64::set_autosave_mode(from_or_default(config_json, "autosave_mode", superman64::AutosaveMode::On));
    superman64::set_camera_invert_mode(from_or_default(config_json, "camera_invert_mode", superman64::CameraInvertMode::InvertY));
    superman64::set_analog_cam_mode(from_or_default(config_json, "analog_cam_mode", superman64::AnalogCamMode::Off));
    superman64::set_analog_camera_invert_mode(from_or_default(config_json, "analog_camera_invert_mode", superman64::CameraInvertMode::InvertNone));
    superman64::set_debug_mode_enabled(from_or_default(config_json, "debug_mode", false));
}
...
void superman64::reset_input_bindings() {
...
void superman64::reset_cont_input_bindings() {
    assign_all_mappings(recomp::InputDevice::Controller, recomp::default_n64_controller_mappings);
}

void superman64::reset_kb_input_bindings() {
    assign_all_mappings(recomp::InputDevice::Keyboard, recomp::default_n64_keyboard_mappings);
}

void superman64::reset_single_input_binding(recomp::InputDevice device, recomp::GameInput input) {
...
bool save_sound_config(const std::filesystem::path& path) {
    ...
    config_json["main_volume"] = superman64::get_main_volume();
    config_json["bgm_volume"] = superman64::get_bgm_volume();
    config_json["low_health_beeps"] = superman64::get_low_health_beeps_enabled();
...
bool load_sound_config(const std::filesystem::path& path) {
    ...
    superman64::reset_sound_settings();
    call_if_key_exists(superman64::set_main_volume, config_json, "main_volume");
    call_if_key_exists(superman64::set_bgm_volume, config_json, "bgm_volume");
    call_if_key_exists(superman64::set_low_health_beeps_enabled, config_json, "low_health_beeps");
    return true;
}

void superman64::load_config() {
    detect_steam_deck();

    std::filesystem::path recomp_dir = superman64::get_app_folder_path();
    ...
    if (!load_controls_config(controls_path)) {
        superman64::reset_input_bindings();
